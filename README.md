...
```
vault_keydir: "{{ inventory_dir }}/vault-password"
...
To enable encryption of secrets set "USE_VAULT=1" in _environments/template/environment.conf_. Any new password,
secrets or private keys generated by the "create_new_environment.sh" will be encrypted. Existing secrets will not be 
changed. To create encrypted secrets you can delete the exiting ones and rerun the script, or you can encrypt them
manually using the _encrypt-file.sh_ script. 

E.g to output the encrypted contents of "environments/password/some_password":

`$ ./scripts/encrypt-file.sh environment/local/vault-password -f environments/password/some_password`
...
You must specify the Ansible vault password explicitely to decrypt the secrets. 

```
ansible-playbook site.yml -i environments/local/inventory  --vault-password-file environments/local/vault-password
```
...
When "USE_VAULT=1" in _environments/template/environment.conf_ the "create_new_environment.sh" script will create a 
password file for the environment. This file contains the secret key that is used to encrypt and decrypt secrets. An existing 
password file will not be overwritten.
...
* An existing secret in a file can be encrypted using _encrypt-file.sh_ script.
  E.g. to encrypt the contents of "/file/with/plaintext/secret" and store it in "environment/secrets/encrypted_secret":
    ...
    `$ ./scripts/encrypt-file.sh environment/local/vault-password -f /file/with/plaintext/secret > environment/secrets/encrypted_secret`
...
* A new random password can be generated using the _gen_password.sh_ script.
  E.g. to generate a new 15 character long encrypted password in "environment/passwords/encrypted_password":
    ...
    `$ ./scripts/gen_password.sh 15 environment/local/vault-password > environment/secrets/encrypted_secret`
...
`$ ./scripts/encrypt-file.sh -d environment/local/vault-password -f environment/password/some_password`
